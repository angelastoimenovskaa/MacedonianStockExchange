...
def filter3(message):
    ...
    def update_data_file(issuer, new_data):
        ...
        if os.path.exists(csv_path):
            ...
            updated_df = pd.concat([df, new_data_df]).drop_duplicates(subset=['Date'], keep='last').sort_values(
                by='Date')
...
pipeline.add([filter1,filter2,filter3])

pipeline.execute()

ending = datetime.now()

time_elapsed = ending - beginning
